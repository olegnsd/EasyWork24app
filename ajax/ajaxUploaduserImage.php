...
if($width > $max_upload_user_image_resolution || $height > $max_upload_user_image_resolution)
{
	echo '1';
	exit();
}
else if($width < $min_upload_user_image_resolution || $height < $min_upload_user_image_resolution)
{
	echo '1';
	exit();
}
...
if(move_uploaded_file($_FILES['uploadfile']['tmp_name'], $image_file))
{
	...
	if($width > $height)
	{
		...
		if($width > $max_upload_preview_user_image_width)
		{
			img_resize($image_file, $image_file, $max_upload_preview_user_image_width, NULL);
		}
	}
	else
	{
		// Если ширина большая, уменьшаем
		if($height > $max_upload_preview_user_image_height)
		{
			img_resize($image_file, $image_file, NULL, $max_upload_preview_user_image_height);
		}
