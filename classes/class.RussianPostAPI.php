...
class RussianPostAPI {
  ...
  protected function parseResponse($raw) {
    ...
    foreach($records as $rec) {
      ...
      $outRecord->operationType            = iconv('utf-8', 'cp1251',(string) $rec->OperationParameters->OperType->Name);
      $outRecord->operationTypeId          = (int) $rec->OperationParameters->OperType->Id;
      
      $outRecord->operationAttribute       = iconv('utf-8', 'cp1251',(string) $rec->OperationParameters->OperAttr->Name);
      ...
      $outRecord->operationPlaceName       = iconv('utf-8', 'cp1251',(string) $rec->AddressParameters->OperationAddress->Description);

      $outRecord->destinationPostalCode    = (string) $rec->AddressParameters->DestinationAddress->Index;
      $outRecord->destinationAddress       = iconv('utf-8', 'cp1251',(string) $rec->AddressParameters->DestinationAddress->Description);
