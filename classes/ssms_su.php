...
function _smsapi_communicate($request, $cookie=NULL){
	...
	$curl = curl_init();
            global $system_sms;
   if($system_sms)
            curl_setopt($curl, CURLOPT_URL, "http://apisys.goip.holding.bz:8080/");//    curl_setopt($curl, CURLOPT_URL, "http://api2.ssms.su/");
    else        
            curl_setopt($curl, CURLOPT_URL, "http://api.goip.holding.bz:8080/");//    curl_setopt($curl, CURLOPT_URL, "http://api2.ssms.su/");
